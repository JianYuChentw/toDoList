...
async function createToDoItems(req, res) {
  ...
  try {
    ...
    if (!canCreateItems) {
      return res.json({ Status: false, message: '新增項目失敗' });
    } else {
      return res.json({ Status: true, message: '新增項目成功' });
    }
  } catch (error) {
...
>>>>>>> turn_right
  ...
  try {
    ...
    if (!isParty) {
      return res.status(200).json({ Status: false, message: '無此項目' });
    }
...
async function deleteToDoItems(req, res) {
  ...
  try {
    ...
    if (!checkHasDeleteItems) {
      return res.status(200).json({ Status: false, message: '刪除項目失敗' });
    }
    //刷新序列
...
async function updateToDoItems(req, res) {
  ...
  if (itemsTitle.length === 0 || itemsTitle === null) {
    return res.json({ Status: true, message: '更新項目輸入不得為空' });
  }
  ...
  try {
    ...
    if (!canUpdatedItemsResult) {
      return res.json({ Status: false, message: '更新項目失敗' });
    } else {
...
async function updatedItemsSchedule(req, res) {
  ...
  try {
    ...
    if (!canChangeScheduleItem) {
      return res.json({ Status: false, message: '更新項目進度失敗' });
    } else {
      return res.json({ Status: true, message: '更新項目進度成功' });
    }
  } catch (error) {
...
async function changeItemSort(req, res) {
  ...
  try {
    ...
    if (!canChangeItemsSortOrder) {
      return res.json({ Status: false, message: '更新項目排序失敗' });
    } else {
      return res.json({ Status: true, message: '更新項目排序成功' });
    }
  } catch (error) {
